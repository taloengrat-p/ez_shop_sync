...
class PinVerifyCubit extends Cubit<PinVerifyState> {
  ...
  PinVerifyCubit({required this.localStorageService}) : super(PinVerifyInitial());

  void doVerify(String value) async {
    final pin = await localStorageService.getSecure(ApplicationConstance.securePINKey);
    final pinSalt = await localStorageService.getSecure(ApplicationConstance.securePINsalt);

    if (pin == null || pinSalt == null) {
      throw ('PIN Data in secure storage invalid');
    }

    final bool isValid = CryptoUtils.verifyPassword(passwordInput: value, passwordHash: pin, salt: pinSalt);
    if (isValid) {
